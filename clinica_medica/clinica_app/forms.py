...
class AppointmentCreateForm(forms.ModelForm):
    ...
    # Define widgets for status and origin fields with some CSS classes
    
    origin = forms.ChoiceField(choices=STATUS_ORIGIN, widget=forms.Select(attrs={'class': 'origin-select'}))
    class Meta:
        model = Appointment
        fields = ['doctor', 'date', 'start_time', 'end_time', 'notes', 'origin']
        widgets = {
            'date': forms.DateInput(attrs={'type': 'date'}),
...
class AppointmentPhoneForm(forms.ModelForm):
    slot_id = forms.IntegerField(widget=forms.HiddenInput())
    STATUS_CHOICES = [
        ('stand_by', 'Stand_by'),
    ...
    # Define widgets for status and origin fields with some CSS classes
    status = forms.ChoiceField(choices=STATUS_CHOICES, widget=forms.Select(attrs={'class': 'status-select'}))
    origin = forms.ChoiceField(choices=STATUS_ORIGIN, widget=forms.Select(attrs={'class': 'origin-select'}))

    class Meta:
        model = Appointment
        fields = ['doctor', 'date', 'start_time', 'end_time', 'notes', 'status', 'origin']
        widgets = {
            'date': forms.DateInput(attrs={'type': 'date'}),
            'start_time': forms.TimeInput(attrs={'type': 'time', 'format': '%H:%M'}),
            'end_time': forms.TimeInput(attrs={'type': 'time', 'format': '%H:%M'}),
    ...
    def save(self, commit=True):
        ...
        # Access the patient associated with the current user (secretary)
        patient = self.request.user.patient
        instance.patient = patient

        # Convert the strings of time into time objects
        start_time = self.cleaned_data['start_time'].strftime('%H:%M')
        end_time = self.cleaned_data['end_time'].strftime('%H:%M')
        instance.start_time = start_time
        instance.end_time = end_time
